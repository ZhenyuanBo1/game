...
for bp_path, mat_asset, label in [
    ...
    if not bp or not mat_asset:
        ...
        continue
    # Walk the Simple Construction Script to find mesh component templates
    assigned = False
    try:
        ...
        print(f"  {label}: SCS walk failed ({e})")
    if assigned:
        unreal.EditorAssetLibrary.save_asset(bp_path)
        print(f"  {label} done")
    else:
        print(f"  {label}: no MeshComponent found — open the Blueprint and drag the material onto slot 0 manually")
